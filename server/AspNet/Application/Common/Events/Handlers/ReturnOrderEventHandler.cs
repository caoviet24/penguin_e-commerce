...
{
    public class ReturnOrderEventHandler(IEmailService emailService, ILogger<ReturnOrderEventHandler> logger) : INotificationHandler<ReturnOrderEvent>
    {
        public async Task Handle(ReturnOrderEvent notification, CancellationToken cancellationToken)
        {
            try
            {
                logger.LogInformation($"Handling order created event for order: {notification.bill.Id}");

                await emailService.SendReturnOrderToBuyerEmailAsync(
                        notification.booth,
                        notification.user,
                        notification.bill
                    );

                await emailService.SendReturnOrderToSalerEmailAsync(
                        notification.saler,
                        notification.booth,
                        notification.user,
                        notification.bill
