...
namespace Application.Product.Queries.GetDeletedByBoothId
{
    public class GetProductsDeletedByBoothIdQuery : IRequest<ResponDataDto>
    {
        public string booth_id { get; set; } = null!;
        public int page_size { get; set; }
        public int page_number { get; set; }
    }
    public class GetProductsDeletedByBoothIdQueryHandler(IDbConnection dbConnection) : IRequestHandler<GetProductsDeletedByBoothIdQuery, ResponDataDto>
    {
        public async Task<ResponDataDto> Handle(GetProductsDeletedByBoothIdQuery request, CancellationToken cancellationToken)
        {
            var productData = await dbConnection.QueryMultipleAsync
            (
                "sp_get_products_deleted_by_booth_id",
                new
                {
