...
namespace Application.Product.Queries.GetDeleted
{
     public class GetProductsDeletedQuery : IRequest<ResponDataDto>
    {
        public int page_size { get; set; }
        public int page_number { get; set; }
    }
    public class GetProductsDeletedQueryHandler(IDbConnection dbConnection) : IRequestHandler<GetProductsDeletedQuery, ResponDataDto>
    {
        public async Task<ResponDataDto> Handle(GetProductsDeletedQuery request, CancellationToken cancellationToken)
        {
            var productData = await dbConnection.QueryMultipleAsync

            (
                "sp_get_products_deleted_pagination",
                new
                {
