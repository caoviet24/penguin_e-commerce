...
namespace Application.Product.Queries.GetActiveByBoothId
{
    public class GetProductsActiveByBoothIdQuery : IRequest<ResponDataDto>
    {
        public string booth_id { get; set; } = null!;
        public int page_size { get; set; }
        public int page_number { get; set; }
    }
    public class GetProductsActiveByBoothIdQueryHandler(IDbConnection dbConnection) : IRequestHandler<GetProductsActiveByBoothIdQuery, ResponDataDto>
    {
        public async Task<ResponDataDto> Handle(GetProductsActiveByBoothIdQuery request, CancellationToken cancellationToken)
        {
            var productData = await dbConnection.QueryMultipleAsync
            (
                "sp_get_products_active_by_booth_id",
                new
                {
