...
export default function EvaluateForm({ bill, onSuccess }: IProps) {
    const [rate, setRate] = React.useState(0);
    const [content, setContent] = React.useState("");

    const createEvaluate = useHookMutation((data) => {
        return productReviewService.create(data);
    });
    const hanldeEvaluateProduct = () => {
        bill.list_sale_bill_detail.forEach(async (b: ISaleBillDetail) => {
            await createEvaluate.mutateAsync({
                product_detail_id: b.product_detail_id,
                rate: rate,
                content: content,
            });
        });
    ...
    return (
        <div className="h-[500px] w-[500px] bg-white p-5 rounded-lg">
            <div className="flex justify-between">
                <h1 className="text-xl font-semibold">Đánh giá sản phẩm</h1>
                <button onClick={onSuccess}>Xác nhận</button>
            </div>
            ...
            <div className="flex flex-col gap-2">
                <div className="flex bg-slate-200 px-4 py-2 gap-4">
                    ...
                    <div className="flex items-center flex-row-reverse">
                        <FaStar size={20} className={`transition-all ${rate < 2 && "text-yellow-500"}`} onClick={() => setRate(1)} />
                        <FaStar size={20} className={`transition-all ${rate < 3 && "text-yellow-500"}`} onClick={() => setRate(2)} />
                        <FaStar size={20} className={`transition-all ${rate < 4 && "text-yellow-500"}`} onClick={() => setRate(3)} />
                        <FaStar size={20} className={`transition-all ${rate < 5 && "text-yellow-500"}`} onClick={() => setRate(4)} />
                        <FaStar size={20} className={`transition-all ${rate < 6 && "text-yellow-500"}`} onClick={() => setRate(5)} />
                    </div>
                </div>
