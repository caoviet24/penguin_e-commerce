...
export default function DeleteAccount({ account, onDeleteSuccess }: { account: IAccount, onDeleteSuccess: () => void }) {

    const [openModal, setOpenModal] = useState(false);
    const [accountData, setAccountData] = useState<IAccount>();

    const { data: dataAccount, isSuccess: isFetchAccountSuccess } = useQuery<IAccount>({
        queryKey: ["get-account-by-id", account],
        queryFn: () => accountService.getById(account?.id),
        enabled: !!account
    });
    ...
    const deleteAccountMutation = useHookMutation((id: string) => {
        return accountService.deleteById(id);
    })

    const handleDeleteAccount = async () => {
        if (accountData?.id) {
            deleteAccountMutation.mutate(accountData.id, {
                onSuccess: () => {
                    onDeleteSuccess();
                }
            });
            ...
            <form className="mx-auto py-4 w-full">
                ...
                <div className='flex flex-row items-center gap-2 mt-4'>
                    ...
                    <label className="inline-flex items-center mb-5 cursor-pointer">
                        <input defaultChecked={accountData?.is_banned|| false} type="checkbox" className="sr-only peer" />
