...
export default function VoucherDetail({ mode, voucher, onSuccess, onClose }: IProps) {

    const [payLoad, setPayLoad] = useState<IPayLoad>({
        voucher_id: voucher.id,
        voucher_type: voucher.voucher_type,
        voucher_name: voucher.voucher_name,
        apply_for: voucher.apply_for,
        after_expiry_date: 0,
        option_expiry_date: "",
        quantity_remain: voucher.quantity_remain,
        quantity_used: voucher.quantity_used,
    ...
    const updateVoucherMutation = useHookMutation((data: any) => {
        return voucherService.update(data);
    });
    ...
    const deleteVoucherMutation = useHookMutation((id: string) => {
        return voucherService.deleteSoft(id);
    });
    ...
    return (
        <div className='w-[600px] p-5 rounded-lg bg-white'>
            ...
            <div className='flex justify-end'>
                {mode === 'edit' && <button onClick={handleUpdateVoucher} className='bg-blue-500 text-white rounded-lg px-5 py-2 mt-5 flex items-center gap-2 hover:bg-blue-300'>
                    <BiPlusMedical />
                    Cập nhật
                </button>}
                {mode === 'delete' && <button onClick={handleDeleteVoucher} className='bg-red-500 text-white rounded-lg px-5 py-2 mt-5'>Xóa</button>}
            </div>
