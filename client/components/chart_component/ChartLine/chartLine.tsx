...
export default function ChartLine({ chartData }: { chartData: ChartData }) {
    const _chartData = {
        labels: chartData?.data?.map((item: { date: string }) => handleTimeVn(item.date)) || [],
        datasets: [
            {
                label: chartData?.lable,
                data: chartData?.data?.map((item: { total: number }) => item.total) || [],
        ...
        <>
            {_chartData &&
                <div className='w-full h-full'>
                    <Line className='!h-[500px] !w-[1000px]' data={_chartData} options={options} />
