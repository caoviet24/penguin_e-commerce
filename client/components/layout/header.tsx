                                    ...
                                    cart.map((item) => (
                                        ...
                                        >
                                            <Image
                                                ...
                                                className="rounded"
                                            />
                                            <p className="flex-1 text-gray-700 truncate">{item.product_detail.product_name}</p>
                                            <p className="text-red-500">{item.product_detail.sale_price.toLocaleString()}đ</p>
                                        </div>
                                    ))
                                ) : (
                                    <div className='h-full w-full flex flex-col items-center justify-center gap-2'>
                                        <Image src="/images/no-rating.png" alt="iphone-13-pro-max" width={100} height={100} />
                                        <p>Giỏ hàng trống</p>
                                    </div>
                                )}
                            </div>
                            <Link href="/cart" className="bg-purple-500 text-white font-medium px-4 py-2 rounded transition float-right mt-1 mr-5">
                                Xem giỏ hàng
                            </Link>
                        </div>
                    </div>
                        ...
                        my_account
                            ...
                            <div className="relative group" >
                                ...
                                <div className='absolute top-full flex-col left-0 bg-white p-2 rounded shadow hidden min-w-64 group-hover:flex'>
                                    <Link className="p-2 hover:bg-purple-500 hover:text-white transition" href={`/account/${my_account.id}`}>
                                        Tài khoản
                                    </Link>
