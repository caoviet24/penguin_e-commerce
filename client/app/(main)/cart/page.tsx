...
export default function Cart() {
    ...
    const { setValue } = useLocationStorage({
        key: 'temp-bill',
        initialValue: {},
    });
    ...
    const handleSelectedAll = () => {
        if (orderItemData && orderSelected.length === orderItemData.length) {
            setOrderSelected([]);
        } else {
            setOrderSelected(orderItemData || []);
        }
    };

    const handleVoucherSelection = (id: string) => {
        const selectedVoucher = voucherData?.find((voucher) => voucher.id === id);
        if (selectedVoucher) {
    ...
    const handleBuyOrder = () => {
        ...
        setValue({
            seller_id: orderSelected[0].booth_id,
            total: totalBill,
    ...
    useEffect(() => {
        ...
        if (voucherSelected) {
            const totalDefault = orderSelected.reduce(
                (total, item) =>
                    total +
                    item.quantity *
                        (item.product_detail.promotional_price > 0
        ...
        <div className="container">
                ...
                <div>
                    {orderItemData &&
                        ...
                        orderItemData.map((item) => (
                            <div
                                key={item.id}
                                className="flex justify-between items-center bg-white mt-4 p-2 rounded-md"
                            >
            ...
            <div className="flex flex-col bg-white mt-4 p-4 rounded-md sticky bottom-0 right-0 mb-10">
                <div className="flex gap-2 justify-end">
                    ...
                    <button onClick={() => setOpenVoucher(true)} className="text-red-500 text-sm">
                        Chọn mã giảm giá
                    </button>
                </div>
                ...
                {voucherSelected && voucherSelected.length > 0 && (
                    <div className="flex flex-col text-end py-1 text-lg">
                        <p>
                            Giá gốc:
                            <span className="opacity-85 line-through text-red-500">
                                {' '}
                                {orderSelected
                                    .reduce(
                                        (total, item) =>
                            ...
                            <div key={voucher.id}>
                                {voucher.voucher_type === 'freeship' ? (
                                    voucher.type_discount === 'percent' ? (
                                        ...
                                        <p>
                                            Giảm tiền giao hàng:
                                            <span className="opacity-85 text-red-500">
                                                {' '}
                ...
                <div className="flex justify-between items-center mt-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            ...
                            className="w-52 py-2 px-10 bg-purple-500 text-white rounded-lg text-center"
                        >
                            Mua hàng
                        </button>
                    </div>
            ...
            >
                <div className="bg-white p-4 rounded-md w-[480px] min-h-32">
                    ...
                    <div className="h-[400px] overflow-y-auto">
                        ...
                        {voucherError && (
                            <div className="flex justify-center items-center h-full">
                                <p className="text-red-500">Lỗi khi tải vouchers: {String(voucherError)}</p>
                            </div>
                        )}
                        ...
                        {!voucherLoading && !voucherError && (
                            <div>
                                {voucherData &&
                                    voucherData.some((voucher) => voucher.voucher_type === 'freeship') && (
                                        <>
                                            <div className="py-2">
                                            ...
                                            {voucherData
                                                ...
                                                .map((voucher) => (
                                                    <Voucher
                                                        key={voucher.id}
                                                        voucher={voucher}
                                                        onChecked={handleVoucherSelection}
                                                    />
                                                ))}
                                        </>
                                    )}
                                ...
                                {(!voucherData || !voucherData|| voucherData.length === 0) && (
                                    <div className="flex justify-center items-center h-full">
                    ...
                    <div className="flex justify-end gap-2">
                        <button
                            ...
                            className="w-40 py-2 px-10 border border-solid border-gray-500 mt-5"
                        >
                            Trở lại
                        </button>
                        <button
                            onClick={handleChooseListVoucher}
                            className="w-40 py-2 px-10 border border-solid border-purple-500 bg-purple-500 text-white mt-5"
                        >
                            Xác nhận
                        </button>
                    </div>
