...
import { ICreateProductDetailPayload } from './productdetail.service';
...
async function getById(id: string): Promise<IProduct> {
    const res = await axiosJWT.get(`/product/get-by-id/${id}`);
    return res.data;
}
...
export interface ICreateProductPayload {
    ...
    product_desc: string;
    category_detail_id: string;
    list_product_detail: ICreateProductDetailPayload[];
}
...
async function active(id: string): Promise<IProduct> {
    const res = await axiosJWT.put(`/product/active/${id}`);
    return res.data;
}

async function inactive(id: string): Promise<IProduct> {
    const res = await axiosJWT.put(`/product/inactive/${id}`);
    return res.data;
}
